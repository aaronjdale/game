function shaderTypeToString(shaderType: number): string {
    switch (shaderType) {
        default:
            return "unknown";
    }
...
export function createShader(gl: WebGL2RenderingContext, shaderType: number, shaderSource: string): WebGLShader {
    ...
    if (!shader) {
        throw new Error(`Failed to create new ${shaderTypeToString(shaderType)} shader.`);
    }
    gl.shaderSource(shader, shaderSource);
    gl.compileShader(shader);
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        const compileError = gl.getShaderInfoLog(shader);
        throw new Error(`Failed to compile ${shaderTypeToString(shaderType)} shader: ${compileError}`);
    }
