...
// build interleaved array from the test data provided
let interleaved: Array<number> = [];
for (let i = 0; i < vertices.length; ++i) {
    const x = vertices[i * 3 + 0];
    const y = vertices[i * 3 + 1];
    const z = vertices[i * 3 + 2];

    const r = colours[i * 3 + 0] / 255;
    const g = colours[i * 3 + 1] / 255;
    const b = colours[i * 3 + 2] / 255;
    const a = 1;

    const u = uv[i * 2 + 0];
    const v = uv[i * 2 + 1];

    interleaved = interleaved.concat([x, y, z, r, g, b, a, u, v]);
}

export const letterGeometry = new Float32Array(interleaved);
